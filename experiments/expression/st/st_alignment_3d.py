...
for t in range(N_EPOCHS):
    ...
    if t % PRINT_EVERY == 0:
        ...
        pd.DataFrame(curr_aligned_coords).to_csv("./out/aligned_coords_st_3d.csv")
        pd.DataFrame(view_idx["expression"]).to_csv("./out/view_idx_st_3d.csv")
        pd.DataFrame(X).to_csv("./out/X_st_3d.csv")
        pd.DataFrame(Y).to_csv("./out/Y_st_3d.csv")
        data.write("./out/data_st_3d.h5")

        if model.n_latent_gps["expression"] is not None:
            curr_W = model.W_dict["expression"].detach().numpy()
            pd.DataFrame(curr_W).to_csv("./out/W_st_3d.csv")
