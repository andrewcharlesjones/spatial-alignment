...
fig, ax = plt.subplots(1, n_views, gridspec_kw={'width_ratios': [1, 1, 1, 1.25]}, figsize=(22, 5))
smoothed_outputs = []
...
for vv in range(n_views):
	...
	plt.scatter(X_grid[:, 0], X_grid[:, 1], c=variances[:, curr_gene_idx], marker="o", s=60, vmin=vmin_var, vmax=vmax_var)
...
vmin, vmax = smoothed_outputs[:, :, GENE_IDX].min(), smoothed_outputs[:, :, GENE_IDX].max()
for vv in range(n_views):
	plt.figure(figsize=(5, 5))
	plt.scatter(X_grid[:, 0], X_grid[:, 1], c=smoothed_outputs[vv][:, GENE_IDX], marker="o", s=60, vmin=vmin, vmax=vmax)
	plt.axis("off")
	plt.savefig("./out/st_slice_smoothed_{}_slice{}.png".format(data.var.gene_ids[GENE_IDX], vv + 1))
	plt.close()
