...
def distance_matrix(X, Y):
	difference = np.expand_dims(X, 0) - np.expand_dims(Y, 1)
...
def gp_likelihood(params):
	...
	X1_warped, X2_warped, kernel_params = unpack_params(params, n1, n2)
	...
	covariance_obs = kernel(X_warped, X_warped, kernel_params) + sigma2 * np.eye(n)
...
if __name__ == "__main__":
	...
	Y_orig = np.vstack([
		mvnpy.rvs(mean=np.zeros(n1), cov=kernel(X_orig, X_orig)) for _ in range(n_genes)
	]).T
	...
	def summary(pars):
		...
		print('LL {0:1.3e}, Penalty {1:1.3e}'.format(gp_likelihood(pars), X1_penalty))
