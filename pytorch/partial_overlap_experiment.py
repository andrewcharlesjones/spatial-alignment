...
def warp_example_experiment():
    ...
    n_samples_list = [n_samples_per_view, n_samples_per_view // 2]
    ...
    for vv in range(n_views):
        ...
        X[view_idx[vv]] = curr_X_observed[: n_samples_list[vv]]

        curr_Y = Y_orig.copy()
        Y[view_idx[vv]] = curr_Y[
            : n_samples_list[vv]
        ]  # + np.random.normal(scale=0.1, size=curr_Y.shape)
    ...
    model = TwoLayerWarpGP(data_dict, distance_penalty_param=1e-4).to(device)
    ...
    X_warped = model.Gs["expression"].detach().numpy()
    ...
    plt.tight_layout()
    plt.savefig("../plots/example_alignment_partial_overlap_simulated.png")
    plt.show()
    import ipdb
