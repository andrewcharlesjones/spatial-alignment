...
if __name__ == "__main__":
    ...
    ## Prep model
    model = TwoLayerWarpGP(data_dict, kernel_func=gp.kernels.Matern32, distance_penalty=False).to(device)
    ...
    def callback(model):
        ...
        for vv in range(N_VIEWS):
            ...
            latent_expression_ax.scatter(
                model.Gs["expression"].detach().numpy()[view_idx_expression[vv], 0],
                model.Gs["expression"].detach().numpy()[view_idx_expression[vv], 1],
                c=np.sum(Y_expression[view_idx_expression[vv], :], axis=1),
                label="View {}".format(vv + 1),
                marker=markers[vv],
                s=100,
            ...
            latent_histology_ax.scatter(
                model.Gs["histology"].detach().numpy()[view_idx_histology[vv], 0],
                model.Gs["histology"].detach().numpy()[view_idx_histology[vv], 1],
                c=Y_histology[view_idx_histology[vv], :],
                label="View {}".format(vv + 1),
                marker=markers[vv],
                s=100,
