...
class VariationalWarpGP(WarpGP):
    ...
    def __init__(self,
        ...
        super(VariationalWarpGP, self).__init__(data_dict,
                                   data_init=True,
                                   n_spatial_dims=2,
                                   n_noise_variance_params=1,
                                   kernel_func=gp.kernels.RBF)
        ...
        if data_init:
            ...
            for ii in range(self.n_views):
                ...
                for mod in self.modality_names:
                    ...
                    curr_modality_and_view_spatial = data_dict[mod]["spatial_coords"][curr_idx, :]
                    curr_X_spatial_list.append(curr_modality_and_view_spatial)
                curr_X_spatial = torch.cat(curr_X_spatial_list, dim=0)
                rand_idx = np.random.choice(np.arange(curr_X_spatial.shape[0]), size=self.m_X_per_view, replace=False)
                Xtilde[ii, :, :] = curr_X_spatial[rand_idx]
            self.Xtilde = nn.Parameter(Xtilde)
        else:
            # Random initialization of inducing locations
            self.Xtilde = nn.Parameter(torch.randn([self.n_views, self.m_X_per_view, self.n_spatial_dims]))
        ...
        self.delta_list = nn.Parameter(torch.randn([self.n_views, self.m_X_per_view, self.n_spatial_dims]))
    ...
    def forward(self, X_spatial, S=1):
        ...
        self.mu_z_G = torch.zeros([self.n_views, self.m_X_per_view, self.n_spatial_dims])
        for vv in range(self.n_views):
            self.mu_z_G[vv] = torch.mm(self.Xtilde[vv], self.mean_slopes[vv]) + self.mean_intercepts[vv]
        
        self.curr_Omega_tril_list = torch.zeros([self.n_views, self.n_spatial_dims, self.m_X_per_view, self.m_X_per_view])
        self.Kuu_chol_list = torch.zeros([self.n_views, self.n_spatial_dims, self.m_X_per_view, self.m_X_per_view])
        G_samples = {}
        for mod in self.modality_names:
        ...
        for ii in range(self.n_views):
            ...
            mu_x_G = torch.mm(curr_X_spatial, self.mean_slopes[ii]) + self.mean_intercepts[ii]
            ...
            for jj in range(self.n_spatial_dims):
                ...
                curr_Omega_G = torch.matmul(curr_Omega_sqt_G, curr_Omega_sqt_G.t()) + 1e-3 * torch.eye(self.m_X_per_view)
                ...
                Kff_diag = self.kernel_G(curr_X_spatial, curr_X_spatial, diag=True) + self.diagonal_offset
                Kuu = self.kernel_G(curr_X_tilde, curr_X_tilde) + self.diagonal_offset * torch.eye(
                    self.m_X_per_view
                )
                ...
                Sigma_tilde = (
                    Kff_diag - aKa + aOmega_a + self.diagonal_offset
                )
                ...
                for mm, mod in enumerate(self.modality_names):
                    ...
                    G_samples[mod][:, self.view_idx[mod][ii], jj] = curr_G_samples[:, curr_idx]
        ...
        for mod in self.modality_names:
            self.curr_Omega_tril_F[mod] = torch.zeros([self.Ps[mod], self.m_G, self.m_G])
        # torch.zeros(
        ...
        for ss in range(S):
            for mod in self.modality_names:
                ...
                Kuu = self.kernel_F(self.Gtilde, self.Gtilde) + self.diagonal_offset * torch.eye(
                    self.m_G
                )
                ...
                for jj in range(self.Ps[mod]):
                    ...
                    curr_Omega = torch.matmul(curr_Omega_sqt, curr_Omega_sqt.t()) + self.diagonal_offset * torch.eye(self.m_G)
