...
if __name__ == "__main__":
	...
	plt.subplot(141)
	plt.scatter(X_orig_layer1[:, 0], X_orig_layer1[:, 1], c=np.log(layer1_raw_df[gene_name] + 1))
	plt.xlabel("Spatial dim 1")
	plt.ylabel("Spatial dim 2")
	plt.title("ST layer 1")

	plt.subplot(142)
	plt.scatter(X_orig_layer2[:, 0], X_orig_layer2[:, 1], c=np.log(layer2_raw_df[gene_name] + 1))
	plt.xlabel("Spatial dim 1")
	plt.ylabel("Spatial dim 2")
	plt.title("ST layer 2")

	plt.subplot(143)
	plt.scatter(X_orig_layer3[:, 0], X_orig_layer3[:, 1], c=np.log(layer3_raw_df[gene_name] + 1))
	plt.xlabel("Spatial dim 1")
	plt.ylabel("Spatial dim 2")
	plt.title("ST layer 3")

	plt.subplot(144)
	plt.scatter(X_orig_layer4[:, 0], X_orig_layer4[:, 1], c=np.log(layer4_raw_df[gene_name] + 1))
	plt.xlabel("Spatial dim 1")
	plt.ylabel("Spatial dim 2")
