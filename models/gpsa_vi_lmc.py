...
class VariationalWarpGP(WarpGP):
    ...
    def compute_mean_and_var(
        ...
        mu_tilde = mu_x.unsqueeze(0) + torch.matmul(alpha_x.transpose(-1, -2), delta - mu_z)

        if len(alpha_x.shape) == 2:
        	a_t_Omega_tril = torch.matmul(alpha_x.transpose(-1, -2).unsqueeze(0), Omega_tril)
        	aOmega_a = torch.sum(torch.square(a_t_Omega_tril), dim=-1)
        	Sigma_tilde = Kff_diag - aKa + aOmega_a + self.diagonal_offset
        else:
        	a_t_Omega_tril = torch.matmul(alpha_x.transpose(-1, -2).unsqueeze(1), Omega_tril.unsqueeze(0))
        	aOmega_a = torch.sum(torch.square(a_t_Omega_tril), dim=-1)
        	Sigma_tilde = Kff_diag.unsqueeze(1) - aKa.unsqueeze(1) + aOmega_a + self.diagonal_offset        
    ...
    def forward(self, X_spatial, view_idx, Ns, S=1, prediction_mode=False):
        ...
        for vv in range(self.n_views):
            ...
            # )
            Kff_diag = torch.ones((curr_X_spatial.shape[0])) * torch.exp(self.warp_kernel_variances[vv])
        ...
        for mod in self.modality_names:
            ...
            #     + self.diagonal_offset
            # )
            Kff_diag = torch.ones((G_samples[mod].shape[:2])) * torch.exp(self.data_kernel_variance)
