...
Y_orig = np.vstack([
	mvnpy.rvs(mean=np.zeros(n1), cov=kernel(X_orig, X_orig, kernel_params_true)) for _ in range(n_genes)
]).T
...
warp_gp = TwoLayerWarpGP(X, Y, n_views=n_views, n_samples_list=[n1, n2], kernel=rbf_covariance)
